                    ...
                    <Transition
                      ...
                      <div className="space-y-1 max-h-60 overflow-y-auto">
                        {group.matches?.map((match) => (
                          <UpdateMatchForm key={match.id} match={match} groupName={group.name} tournamentId={tournament.id} />
                        ))}
                      </div>
                    </Transition>
                  </div>
              ...
              <div className="flex flex-col gap-2 justify-center">
                {tournamentPhase.firstMatch && (
                  <div className="flex gap-5 items-center justify-between flex-wrap">
                    <h3 className="flex-shrink-0 text-left">
                      {tournamentPhase.firstMatchGroup?.name ?? `Group ${tournamentPhase.firstMatch.tournamentGroupId}`}
                    </h3>
                    <div className="flex-1">
                      <UpdateMatchForm match={tournamentPhase.firstMatch} groupName={tournamentPhase.firstMatchGroup?.name} tournamentId={tournament.id} />
                    </div>
                  </div>
                )}
                {tournamentPhase.secondMatch && (
                  <div className="flex gap-5 items-center justify-between flex-wrap">
                    ...
                    <div className="flex-1">
                      <UpdateMatchForm match={tournamentPhase.secondMatch} groupName={tournamentPhase.secondMatchGroup?.name} tournamentId={tournament.id} />
                    </div>
                  </div>
                )}
              </div>
