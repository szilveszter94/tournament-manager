...
export default function TournamentGroups({ tournament, tournamentId }: TournamentGroupsProps) {
    ...
    return (
      tournament.phases
        ...
        ?.map((phase) => {
          ...
          const secondMatchGroup = secondMatch
            ? phase?.groups?.find((g) => g.id === secondMatch.tournamentGroupId)
            : null;
        ...
        <div className="space-y-4">
          {filteredPhases.map((phase) => (
            <div key={phase.id}>
              ...
              {phase.firstMatch || phase.secondMatch ? (
                    ...
                    {phase.firstMatch && (
                      <div className="flex gap-5 items-center justify-between flex-wrap">
                        <h3 className="flex-shrink-0 text-left">
                          {phase.firstMatchGroup?.name ?? `Group ${phase.firstMatch.tournamentGroupId}`}
                        </h3>
                        <div className="flex-1">
                          <UpdateMatchForm match={phase.firstMatch} tournamentId={tournamentId} />
                        </div>
                      </div>
                    )}
                    {phase.secondMatch && (
                      <div className="flex gap-5 items-center justify-between flex-wrap">
                        <h3 className="flex-shrink-0 text-left">
                          {phase.secondMatchGroup?.name ?? `Group ${phase.secondMatch.tournamentGroupId}`}
                        </h3>
                        <div className="flex-1">
                          <UpdateMatchForm match={phase.secondMatch} tournamentId={tournamentId} />
                        </div>
                      </div>
                    )}
                  </div>
